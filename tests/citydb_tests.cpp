...
TEST_CASE("city database reconciles tree-sitter snapshot into semantic tables", "[citydb]")
{
    ...
    // Ordered by display_name: IView, PlainData, Tower, build_city
    CHECK(classes[0].module_path == "src");
    CHECK(classes[0].qualified_name == "IView");
    ...
    CHECK(classes[3].qualified_name == "build_city");
    CHECK(classes[3].entity_kind == "tree");
    CHECK(classes[3].building_functions == 1);
    REQUIRE(classes[3].function_sizes.size() == 1);
    CHECK(classes[3].function_sizes[0] == 5); // line 45 - line 40
