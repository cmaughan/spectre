...
void run_nvim_crash_tests()
{
    ...
    run_test("nvim rpc unblocks promptly when the child process is force-killed mid-request", []() {
        ScopedEnvVar env("DRAXUL_RPC_FAKE_MODE", "hang");

        NvimProcess process;
        expect(process.spawn(fake_server_path()), "fake RPC server spawns in hang mode");
        expect(process.is_running(), "fake server should be running after spawn");
        ...
        expect(elapsed < std::chrono::seconds(2),
            "rpc.close() must unblock an in-flight request within 2 seconds after process kill");

        rpc.shutdown();

        expect(!process.is_running(), "process should not be running after shutdown");
    });

    // Test 2: After a simulated crash, NvimRpc::shutdown() joins the reader thread without deadlock.
    run_test("nvim rpc reader thread exits cleanly after process is force-killed", []() {
        ScopedEnvVar env("DRAXUL_RPC_FAKE_MODE", "hang");
        ...
        expect(elapsed < std::chrono::seconds(2),
            "rpc.shutdown() must complete within 2 seconds after the child process dies");
    });
    ...
    run_test("nvim rpc emits a pipe error log when the child process dies unexpectedly", []() {
        ...
        expect(!process.is_running(), "is_running() must return false after shutdown");
        ...
        expect(found_pipe_error || found_transport_closed,
            "log should contain a pipe error or transport-closed message after child death");
    });
    ...
    run_test("nvim rpc reader thread exits after SIGKILL on the child process (POSIX)", []() {
        ScopedEnvVar env("DRAXUL_RPC_FAKE_MODE", "hang");
        ...
        expect(elapsed < std::chrono::seconds(2),
            "requester thread must unblock within 2 seconds after SIGKILL");

        rpc.shutdown();

        expect(!process.is_running(), "process must not be running after SIGKILL");
    });
#endif
}
