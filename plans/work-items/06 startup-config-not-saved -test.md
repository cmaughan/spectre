...
- [ ] Read existing tests under `tests/` to understand the test harness and fixture patterns.
- [ ] Read `app/app.cpp` `initialize()` and `shutdown()` to identify the config-save path.
- [ ] Write a test in the `draxul-tests` target (or a new file `tests/startup_config_tests.cpp`):
...
- [ ] Also test: a *successful* init followed by a config change and normal exit does persist config (positive case).
- [ ] Add the new test file to `tests/CMakeLists.txt`.
- [ ] Run `cmake --build build --target draxul-tests && ctest --test-dir build -R draxul-tests`.
