...
- [ ] Introduce explicit branch descriptors and per-branch ring records.
- [ ] Separate descriptor generation from mesh emission.
- [ ] Keep generation deterministic via stable seed hashing.

### Phase 4: Recursive branch growth

- [ ] Generate child branch counts from configurable ranges.
- [ ] Spawn child branches from local parent-ring patches.
- [ ] Scale child length/radius relative to the parent branch.
- [ ] Add branch curvature and upward/outward bias controls.

### Phase 5: Branch attachment stitching

- [ ] Build local stitched junctions between parent branch patches and child root rings.
- [ ] Smooth normals near the branch roots so junctions read as growth instead of pipe intersections.
- [ ] Keep UV and tangent continuity consistent enough for bark materials.

### Phase 6: Megacity rendering integration

- [ ] Choose the first integration model:
  - [ ] prototype shared tree meshes
  - [ ] or dynamic procedural mesh registration/cache
- [ ] Remove shader-generated UV/tangent conventions from Megacity materials and rely on mesh-provided data instead.
- [ ] Replace placeholder cube trees in Megacity with generated tree meshes.
- [ ] Extend Megacity vertex/render paths if tangents or custom mesh handles require it.
...
- [ ] Recursive branch generation produces obviously tree-like forms rather than just cylinders.
- [ ] Megacity can render at least one generated tree mesh instead of placeholder cube trees.
