...
- [ ] Read `app_config_io.cpp` and find the full config path construction logic.
- [ ] Confirm whether `APPDATA` is used directly or combined via `std::filesystem::path`.
- [ ] Check whether the same file has similar issues for macOS (`HOME`, `XDG_CONFIG_HOME`).

## Fix strategy

- [ ] After the null-check, add an empty-check:
  ```cpp
  ...
  ```
- [ ] Apply the same pattern to any other environment variable reads in the same file (`HOME`, `XDG_CONFIG_HOME`).
- [ ] Ensure the fallback path is a sensible absolute path (e.g. the user's home directory or the executable directory).

## Acceptance criteria

- [ ] Setting `APPDATA=""` (or unsetting it) before launching does not produce a config file in the working directory.
- [ ] A `WARN` log is emitted when the env var is empty.
- [ ] `07 config-missing-malformed -test` exercises this code path.
