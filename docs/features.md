...
- **Session-scoped shell restore CLI**: `--session <id>` selects which shell session Draxul should attach to or restore, `--list-sessions` prints known sessions with live/detached/saved status and workspace/pane counts, `--attach-session` explicitly activates a running session, and `--kill-session` explicitly kills a live session or deletes its saved topology.
