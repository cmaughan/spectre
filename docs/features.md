...
- **MegaCity performance preview mode**: The City Build UI now includes a saved `Perf Mode` toggle that blends flat-color buildings toward a green-to-red heat palette per semantic building layer, driven by a separate live metrics snapshot for every building and function and indexed in the shader by stable building/layer ids
