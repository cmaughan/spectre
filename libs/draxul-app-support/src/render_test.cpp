...
std::optional<RenderTestScenario> load_render_test_scenario(const std::filesystem::path& path, std::string* error_message)
{
    ...
    if (auto name = toml_support::get_string(*document, "name"))
        scenario.name = *name;
    if (auto width = toml_support::get_int(*document, "width"))
        scenario.width = static_cast<int>(*width);
    if (auto height = toml_support::get_int(*document, "height"))
        scenario.height = static_cast<int>(*height);
    if (auto font_size = toml_support::get_double(*document, "font_size"))
        scenario.font_size = static_cast<float>(*font_size);
    else if (auto font_size_int = toml_support::get_int(*document, "font_size"))
        scenario.font_size = static_cast<float>(*font_size_int);
    if (auto timeout_ms = toml_support::get_int(*document, "timeout_ms"))
        scenario.timeout_ms = static_cast<int>(*timeout_ms);
    if (auto settle_ms = toml_support::get_int(*document, "settle_ms"))
        scenario.settle_ms = static_cast<int>(*settle_ms);
    if (auto pixel_tolerance = toml_support::get_int(*document, "pixel_tolerance"))
        scenario.pixel_tolerance = static_cast<int>(*pixel_tolerance);
    if (auto changed_pixels_threshold_pct = toml_support::get_double(*document, "changed_pixels_threshold_pct"))
        scenario.changed_pixels_threshold_pct = *changed_pixels_threshold_pct;
    if (auto display_ppi = toml_support::get_double(*document, "display_ppi"))
        scenario.display_ppi = static_cast<float>(*display_ppi);
    if (auto debug_overlay = toml_support::get_bool(*document, "debug_overlay"))
        scenario.debug_overlay = *debug_overlay;
    if (auto enable_ligatures = toml_support::get_bool(*document, "enable_ligatures"))
