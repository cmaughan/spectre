...
std::string module_path_for_file(std::string_view file_path)
{
    ...
    // If the path is just a filename (no directory), group into a single module.
    ...
    try
    {
        ...
        for (const auto& file : snapshot.files)
        {
            ...
            for (const auto& sym : file.symbols)
            {
                ...
                if ((sym.kind == SymbolKind::Class || sym.kind == SymbolKind::Struct || is_free_function) && !sym.fields.empty())
                {
...
std::vector<std::string> CityDatabase::list_modules() const
{
    ...
    try
    {
        Statement stmt(impl_->db.get(),
            ...
            "WHERE entity_kind IN ('building', 'tower', 'block', 'tree') ORDER BY module_path");
...
std::vector<CityClassRecord> CityDatabase::list_classes_in_module(std::string_view module_path) const
{
    ...
    try
    {
        Statement stmt(impl_->db.get(),
            ...
            "WHERE ce.module_path = ? AND ce.entity_kind IN ('building', 'tower', 'block', 'tree') "
        ...
        while (stmt.step() == SQLITE_ROW)
        {
            ...
            row.is_struct = read_text(11) == "struct" && row.building_functions == 0;

            // For free functions (tree entities), synthesize a single function
            // entry from the symbol's line range so the building gets height.
            if (row.entity_kind == "tree")
            {
                const int line_start = sqlite3_column_int(stmt.raw(), 12);
                const int line_end = sqlite3_column_int(stmt.raw(), 13);
                const int line_count = std::max(1, line_end - line_start);
